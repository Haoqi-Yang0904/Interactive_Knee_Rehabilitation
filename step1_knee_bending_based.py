...
FEEDBACK_MESSAGES = {
    "far": [
        "继续屈膝。先把身子放稳，膝盖再往里弯一点，不着急。",
        "继续屈膝。对，就是这样。腿慢慢弯，腰别使劲，哪里疼就先停一下。",
        "继续屈膝。先找舒服的劲儿。膝盖一点点往回收，不用一下弯很多。",
    ],
    "mid": [
        "继续屈膝。不错，离目标近了。身子别晃，膝盖再慢慢弯一点。",
        "继续屈膝。很好，再来一点点。别憋气，慢慢弯，疼了就停。",
        "继续屈膝。现在做得挺稳。脚跟跟着动，膝盖再往里收一点。",
    ],
    "close": [
        "继续屈膝。快到了，就差一点。别猛用力，再轻轻弯一点。",
        "继续屈膝。很好，很接近了。身子稳住，膝盖再往里收一点点。",
        "继续屈膝。做得不错，别着急。慢慢弯，慢慢停，动作越稳越好。",
    ],
    "near": [
        "继续屈膝。就差一点点了。先别急，再慢慢弯一点点就够了。",
        "继续屈膝。快到了，膝盖再轻轻弯一点。别憋气，也别硬拉。",
        "继续屈膝。很好，就这个劲儿。再往里收一点点，稳住。",
    ],
    "target": [
        "保持住。很好，已经到一百二十度了。先别放下，稳住，慢慢呼吸。",
        "保持住。达标了，很棒。现在不用再硬弯了，保持住就行。",
        "保持住。标准到了，做得很好。停在这个位置，坚持一下，别憋气。",
    ],
    "hold": [
        "保持住。保持得很好，再坚持一下。要是疼得厉害，就慢慢放下来。",
        "保持住。很棒，继续稳住。肩膀放松，腰也别使劲。",
        "保持住。状态不错，再坚持几秒。慢慢呼吸，不要憋着。",
        "保持住。做得好，就这样稳住。累了可以慢慢放下，不要突然松。",
    ],
    "rest": [
        "慢慢放下。保持时间到了，做得很好。现在可以休息一下。",
        "慢慢放下。这次保持完成了，很棒。请放松休息。",
        "慢慢放下。已经坚持够了，做得不错。现在准备休息。",
    ],
    "adjust_view": [
        "调整位置。请把身子转成侧面对着摄像头，不要斜着。",
        "调整位置。现在看角度可能不准。您先侧过身来，调整好了咱们再继续。",
    ],
    "low_visibility": [
        "调整位置。我现在看这条腿不太准。请让大腿、膝盖和小腿下端清楚一点。",
        "调整位置。腿部关键点有点不稳。把膝盖以下露出来一点，我再帮您看角度。",
    ],
}
...
def choose_tracked_leg(landmarks):
    ...
    return max(leg_scores, key=leg_scores.get)
...
def get_best_distal_landmark(landmarks, leg_side, require_visible=False):
    best_part_name = None
...
def build_direction_hint(current_distal, target_distal):
    ...
    if direction_text:
        return f"继续屈膝，小腿向{direction_text}收，尽量贴近辅助线"
    return "继续屈膝，小腿沿箭头和辅助线方向慢慢收回来"
...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        if results.pose_landmarks and results.pose_landmarks.landmark:
            ...
            _, distal_landmark_name = get_best_distal_landmark(
                landmarks,
                tracked_leg,
        ...
        if angle is not None:
            ...
            info_lines.append("请先进入画面并露出整条腿")

        if results.pose_landmarks is None:
            action_lines = [
                "还没有识别到完整人体，请侧身进入画面",
                "请把髋部、膝盖和膝盖以下的一点都放进镜头里",
            ]
            action_panel_style = {
            ...
            action_lines = [
                "保持住，先不要放下",
                f"再坚持 {remaining_seconds:.1f} 秒，就可以慢慢放松休息",
            ]
            action_panel_style = {
                "text_fill": (235, 255, 242, 255),
                "panel_fill": (20, 112, 71, 210),
            ...
            action_lines = [
                guidance_text or "继续屈膝，小腿沿辅助线方向慢慢收回来",
